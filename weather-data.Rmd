---
title: "Time-series weather data for Vale de Cavalos, Portugal"
author:
...
This notebook summarizes time series data, starting in 2009 until present, from an automated weather station located in the village of Vale de Cavalos, Santarem, Portugal. The weather station contains sensors to measure... + brand of each sensor + querying of the data + add a nice photo

The weather data available through this weather station, and summarized in this notebook, is currently being used to (1) inform decision support tools for disease management in vineyards and (2) monitor the occurrence of extreme weather events in the region. The same data could be further used to estimate water-limited production yields for the main crops cultivated in the region with crop growth models, among other applications in the field of agronomy. This notebook is updated yearly as data become available.

<hr style="border:1.5px solid black">
# **Location of weather station**

The weather station is located in the village of Vale de Cavalos with the following GPS coordinates: .... latitude, .... longitude, ... m above sea level. This location encompasses a vineyard in a lowland area adjacent to the village in a transitional zone between heath (charneca) and alluvial plains (campina), approximately 5 km south of Tejo river. The transitional zone can be best visualized by zooming-out from the location of the station displayed in the map below. The alluvial plains around river Tejo comprise important agricultural regions for the production of arable crops (e.g., maize and tomato) as well as for wine production. 
...
The region is characterized by a Mediterranean climate (Koppen etc.) with dry and hot summer periods between June and September, and weat and cold winter periods between December and March.

Vale de Cavalos has ... inhabitants etc.
...
# plot 1
data <- read.csv('D:/# Jvasco/Visualization/Vale de Cavalos/rmarkdown-weather/data/Weather Data.csv')
data_aggr <- aggregate(data[c('IRRAD_MJm2day', 'UM')], by=list('year'=data$Year, 'month'=data$Month), FUN=mean, na.rm=T)
ts_data_rad <- xts(data[,48], order.by=as.Date(data[,2], "%Y-%m-%d"))
ts_data_hum <- xts(data[,10], order.by=as.Date(data[,2], "%Y-%m-%d"))
...
ts_data_tmin <- xts(data[,3], order.by=as.Date(data[,2], "%Y-%m-%d"))
ts_data_tmax <- xts(data[,4], order.by=as.Date(data[,2], "%Y-%m-%d"))
...
ts_data_rain <- xts(data[,16], order.by=as.Date(data[,2], "%Y-%m-%d"))
ts_data_et0 <- xts(data[,27], order.by=as.Date(data[,2], "%Y-%m-%d"))
...
rain <- dygraph(ts_rain, group=ts_rad) %>%
  ...
  dyOptions(fillGraph=T, drawPoints=T, pointSize=2) %>%
  dyHighlight(highlightCircleSize=4, highlightSeriesBackgroundAlpha = 0.5, highlightSeriesOpts = list(), hideOnMouseOut = TRUE) %>%
  dyRangeSelector(height = 40, dateWindow = c("2021-01-01", "2021-12-31"))
